...
# - Final article sections are context-aware and structurally mandatory.
# - Images are grounded in article content and must stay outside Markdown tables.
# - Technical diagrams must not contain fake paragraphs or invented prose.
# - Editorial validation must protect technical accuracy and avoid unsupported performance claims.
...
4. FINAL SECTION IS MANDATORY:
   - The plan MUST contain one dedicated final closing section.
   - The closing section MUST be the final planned section.
   - It MUST use a context-aware, topic-specific professional heading.
   - Do NOT automatically use `Key Takeaways`, `Final Thoughts`, or `Summary`.
   - Use those generic headings only when they are genuinely the best fit for the article.
   - The closing section must synthesize the article's engineering thesis and answer:
     "After reading this article, what should an engineer now understand differently?"
   - It must not introduce unsupported technical claims or simply repeat earlier bullet points.
   - Never allow the article to terminate immediately after an ordinary technical section.

5. Task Details: For every section specify:
...
5. Inline Placement:
   ...
   - NEVER place an image placeholder between a Markdown table header row and its separator/data rows.
   - If a visual belongs to a section containing a table, place the complete table first, then the image placeholder and caption.
...
- Text Discipline:
  * NEVER generate fake paragraphs, Lorem Ipsum, pseudo-text, placeholder prose, or invented article excerpts.
  * Use only the figure title, major component labels, short 1-4 word sublabels, and essential technical terms explicitly supported by the article.
  * Do not place explanatory sentences or text walls inside cards.
  * Prefer visual relationships, arrows, icons, grouping, and concise labels over prose.
